...
Beneficiaries of humanitarian action formed `r round(filter(fsc_se, humanitarian_or_development == "Humanitarian") %>% {sum(.$new_beneficiaries, na.rm = TRUE)} / sum(fsc_se$new_beneficiaries, na.rm = TRUE) * 100, digits = 2)`% of the `r sum(fsc_se$new_beneficiaries) %>% format(big.mark = ",")` beneficiaries in the first quarter of 2022. The remainder were reached through development interventions. 
...
fsc_se %>% 
  ...
  # Putting in the 0s for SO2 here, similar to the chunk above
  rbind(tribble(~strat_obj, ~was_nutrition_mainstreamed_in_activity, ~beneficiaries, 
                "SO_2","No", 0, 
                "SO_2", "Yes", 0)) %>%
  pivot_wider(names_from = strat_obj, values_from = beneficiaries) %>% 
  mutate(total_beneficiaries = SO_1 + SO_3, 
         `%_beneficiaries` = round(total_beneficiaries / sum(total_beneficiaries) * 100, digits = 2)) %>% 
  relocate(SO_2, .after = SO_1) %>% 
...
As in 2021, the number of beneficiaries reached has been heavily biased towards relatively few areas, which is not appropriate for a unionwide response. A total of `r distinct(fsc_se, admin3_pcode) %>% nrow()` townships have been reached across `r distinct(fsc_se, admin1_pcode) %>% nrow()` states/regions in Southeast Hub. 
...
fsc_se %>%  
  ...
  ungroup() %>% 
  right_join(pin %>% 
               # aung, remember to use a filter for the hub variable if you use the pin df
               filter(hub == "southeast") %>%  
               group_by(state, admin1_pcode) %>% 
...
fsc_se %>% 
  ...
  kable(caption = "Townships reached by number of beneficiaries", 
        format.args = list(big.mark = ",")) %>%  
  # Removing the footnote, since you're showing all the townships 
  # footnote("Only townships with more than 3% of the total beneficiaries are shown", 
  #          general_title = "") %>%  
  kable_classic_2("striped")
...
fsc_se %>%
  ...
  # This is how you change the colour of the lines
...
Crop, vegetable and seed kits formed the largest group of agricultural activities and reached `r filter(fsc_se, activity_red == "crop, vegetable and seed kits") %>% {sum(.$households)} %>% format(big.mark = ",")` households (for a full breakdown by agricultural activity, please refer to the plot below). 
...
# you need to create the labels dataset first 
ag_labels <- fsc_se %>% 
  mutate(agricultural_activity = ifelse(activity_red %in% c("crop, vegetable and seed kits", 
                                                            "FFS and farmer training", 
                                                            "IGA and small grants", 
...
fsc_se %>% 
  ...
  mutate(delivery_modality = case_when(str_detect(delivery_modality, "Service") ~ "Service delivery", TRUE ~ delivery_modality)) %>% 
  ...
  left_join(fsc_se %>%
          filter(!is.na(delivery_modality)) %>%
          mutate(delivery_modality = case_when(str_detect(delivery_modality, "Service") ~ "Service delivery", 
  ...
  mutate_at(vars(`In-kind`, `Service delivery`, `CBT/CVA`), ~recode(., "-" = NA_character_)) %>% 
...
avg_transfer_value %>%  
  ...
  column_spec(4, color = "white", background = spec_color(avg_transfer_value$avg_transfer_value[1:7], 
                                                          end = 0.8, direction = -1)) %>%
...
```{r partner-cash-values, fig.height=6.5, eval=FALSE}
# I put eval=FALSE in the header of this chunk. It will no longer run
# this is a function that creates plots
...
# pasting plots together using the patchwork package
fsc_se %>% partner_mean_usd("food distribution") +
  labs(title = "Food distributions (per person)") +
  
fsc_se %>% partner_mean_usd("food_cash for work_assets") + 
  labs(title = "Food/cash for work/assets (per hhd)") + 
  
fsc_se %>% partner_mean_usd("multi-purpose cash transfer") +
  labs(title = "Multi-purpose cash transfer (per hhd)") + 
  
# commented out because there is no IGA in the northwest  
# fsc_se %>% partner_mean_usd("IGA and small grants") +
...
fsc_se %>% 
  # you might want to filter out any activities where there are too few distributions 
...
Overall, `r round((filter(fsc_se, usd_per_person >= (114.55 / 5) & activity_red == "food distribution") %>% {sum(.$new_beneficiaries)}) / (filter(fsc_se, !is.na(usd_per_person) & activity_red == "food distribution") %>% {sum(.$new_beneficiaries)}) * 100, digits = 2)`% of food distribution beneficiaries have received at least 100% of the MEB and `r round((filter(fsc_se, usd_per_person >= (114.55 / 5 / 2) & activity_red == "food distribution") %>% {sum(.$new_beneficiaries)}) / (filter(fsc_se, !is.na(usd_per_person) & activity_red == "food distribution") %>% {sum(.$new_beneficiaries)}) * 100, digits = 2)`% have received at least 50% of the MEB. 


```{r table-meb-usd-hhd-bin}

food_bins <- fsc_se %>% filter(activity_red %in% c("food distribution") & 
...
fsc_se %>% 
  ...
  footnote(general = "Only persons reached through CBT/CVA/hybrid modalities are included",
           general_title = "")
# pander(caption = "USD values of food distributions by percentage of MEB received")
...
`r round(filter(fsc_se, beneficiary_type == "Host/local Community") %>%  {sum(.$new_beneficiaries)} / sum(fsc_se$new_beneficiaries) * 100, digits = 2)`% of beneficiaries were from the host or local community. `r round(filter(fsc_se, beneficiary_type == "Internally Displaced") %>%  {sum(.$new_beneficiaries)} / sum(fsc_se$new_beneficiaries) * 100, digits = 2)`% beneficiaries were IDPs. 
...
There are `r fsc_se %>% distinct(org_code) %>% nrow()` partners that were involved in direct implementation that have reported achievements in first quarter of 2022. These implementing partners corresponded to a total of `r fsc_se %>%  distinct(report_org_code) %>% nrow()` reporting organisations. The largest reporting organisation, org_9566, had reached vast majority of beneficiaries in 10 townships in 4 states.   
...
fsc_se %>% 
  ...
  right_join(pcode3_shape %>%
               filter(state %in% c("Kayah", "Kayin", "Mon", "Tanintharyi")), 
             by = c("admin3_pcode")) %>% 
  st_as_sf() %>% 
  ggplot() +
  geom_sf(aes(fill = beneficiaries), size = 0.1) +
  geom_sf(data = pcode1_shape %>%  filter(state %in% c("Kayah", "Kayin", "Mon", "Tanintharyi")),
          alpha = 0, colour = "black", size = 0.5) + 
  scale_fill_viridis_c(direction = -1, trans = "log10", 
                       # adding a 30000 break
                       breaks = c(100, 1000, 10000, 30000, 100000, 500000),
                       labels = comma) + 
  theme_void() +
...
pin %>% 
  ...
  scale_fill_viridis_c(direction = -1, trans = "log10",  
                       # adding a 30000 break
                       breaks = c(100, 1000, 10000, 30000, 100000, 500000),
                       # Removed on line here that was affecting putting the colours on the same scale
                       labels = comma) + 
  theme_void() + 
  theme(legend.text = element_text(size = 10),
          legend.title = element_text(size = 10),
